...
void set_current_target(target* t) {
	cur_target = t;
}
...
int gdb_main_loop(struct target_controller *tc, bool in_syscall)
{
	...
	while(1) {
		...
		case 'g': { /* 'g': Read general registers */
			ERROR_IF_NO_TARGET();
			uint32_t armRegsSize = target_regs_size(cur_target);
			uint8_t* arm_regs = _alloca(armRegsSize);
			target_regs_read(cur_target, arm_regs);
			gdb_putpacket(hexify(pbuf, arm_regs, armRegsSize), armRegsSize * 2);
			break;
			}
		...
		case 'G': {	/* 'G XX': Write general registers */
			ERROR_IF_NO_TARGET();
			size_t armRegsSize = target_regs_size(cur_target);
			uint8_t arm_regs = _alloca(armRegsSize);
			unhexify(arm_regs, &pbuf[1], armRegsSize);
		...
		case 'P': { /* Write single register */
			...
			uint8_t* val = _alloca(strlen(&pbuf[n])/2);
